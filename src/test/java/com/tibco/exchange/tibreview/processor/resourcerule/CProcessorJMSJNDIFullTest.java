...
public class CProcessorJMSJNDIFullTest {
	...
	public void testCProcessorJMSJNDIFullTest() {
		...
		try {
			...
			fileresource = new TIBResource("src/test/resources/FileResources/JMSConnectionResourceJNDIFull.jmsConnResource");
			fileresource.toString();
			System.out.println(fileresource.toString());
			assertTrue(fileresource.toString().equals("TIBResource [filePath=src/test/resources/FileResources/JMSConnectionResourceJNDIFull.jmsConnResource, type=jms:JMSConnectionFactory]"));
			Resourcerule rule = new Resourcerule();
			
			Tibrules tibrules= RulesParser.getInstance().parseFile("src/test/resources/FileResources/xml/JMSConnectionResourceJNDIFull.xml");
