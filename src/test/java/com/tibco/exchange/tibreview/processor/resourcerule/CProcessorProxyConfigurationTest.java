...
public class CProcessorProxyConfigurationTest {
	...
	public void testCProcessorLDAPConfigurationTest() {
		...
		try {
			...
			fileresource = new TIBResource("src/test/resources/FileResources/ProxyConfigurationResource.httpProxyResource");
			fileresource.toString();
			System.out.println(fileresource.toString());
			assertTrue(fileresource.toString().equals("TIBResource [filePath=src/test/resources/FileResources/ProxyConfigurationResource.httpProxyResource, type=httpproxy:ProxyConfiguration]"));
			Resourcerule rule = new Resourcerule();
			
			Tibrules tibrules= RulesParser.getInstance().parseFile("src/test/resources/FileResources/xml/ProxyConfigurationResource.xml");
